...
def get_peft_model_state_dict(
    ...
    elif config.peft_type in (PeftType.LORA_A, PeftType.NASLORA, PeftType.LOCP):
        bias = config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "lora_" in k}
        elif bias == "all":
    ...
    elif config.peft_type == PeftType.HSSA:
        # todo (djw has done): need to modify
        # has add `modules_weights_` for save the model parameters
        to_return = {
            k: state_dict[k]
            for k in state_dict
            if "hssa_lambda_" in k or "modules_weights_" in k
        }
        if config.save_projection:
            # TODO: adding hssa_A and hssa_B to `self.get_base_layer` would
            # make name to match here difficult to predict.
            if f"base_model.hssa_A.{adapter_name}" not in state_dict:
                raise ValueError(
                    "Model was initialised to not save hssa_A and hssa_B but config now specifies to save projection!"
                    " Set `config.save_projection` to `False`."
                )
            to_return["base_model.hssa_A." + adapter_name] = state_dict[
                "base_model.hssa_A." + adapter_name
            ]
            to_return["base_model.hssa_B." + adapter_name] = state_dict[
                "base_model.hssa_B." + adapter_name
            ]
    else:
...
def set_peft_model_state_dict(
    ...
    if config.peft_type in (
        ...
        parameter_prefix = {
            ...
            PeftType.LORA_A: "lora_",
            PeftType.NASLORA: "lora_",
            PeftType.LOCP: "lora_",
            PeftType.HSSA: "hssa_lambda_",  # todo (djw): need to modify
        }[config.peft_type]
        for k, v in state_dict.items():
        ...
        if config.peft_type == PeftType.ADALORA:
            ...
            if (
                config.save_projection
                and "base_model.hssa_A" not in peft_model_state_dict
            ):
                raise ValueError(
                    "Specified to load hssa_A and hssa_B from state dictionary however they were not present!"
                )
            elif (
                not config.save_projection
                and "base_model.hssa_A" in peft_model_state_dict
            ):
                warnings.warn(
                    "Specified to not load hssa_A and hssa_B from state dictionary however they are present in state"
                    " dictionary! Consider using them to ensure checkpoint loading is correct on all platforms using"
                    " `peft_config.save_projection = True`"
                )
            elif not config.save_projection:  # and no hssa_A in state dictionary
                warnings.warn(
                    "Specified to not load hssa_A and hssa_B from state dictionary. This means we will be relying on"
                    " PRNG initialisation to restore these projections using `config.projection_prng_key`, which may"
                    " not be accurate on all system configurations."
                )
