...
from .layer import LoCPLayer, Linear8bitLt, Linear

if is_bnb_available():
    import bitsandbytes as bnb


class LoCPModel(torch.nn.Module):
    """
    ...
    Example:
        ...
        >>> from peft import LoCPModel, LoraConfig
        ...
        >>> lora_model = LoCPModel(config, model)
        ```
    ...
    **Attributes**:
    ...
    def __init__(self, model, config, adapter_name):
        ...
        self.layer_number = self.model.config.num_hidden_layers
        self.adaptive_layer_weights = nn.Parameter(torch.ones(self.layer_number))
    ...
    def add_adapter(self, adapter_name, config=None):
        ...
        if len(self.peft_config) > 1 and self.peft_config[adapter_name].bias != "none":
            raise ValueError(
                "LoCPModel supports only 1 adapter with bias. When using multiple adapters, set bias to 'none' for all adapters."
            )
    ...
    def _find_and_replace(self, adapter_name):
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if isinstance(target, LoCPLayer):
                    target.update_layer(
                        adapter_name,
                        lora_config.r,
    ...
    def _set_adapter_layers(self, enabled=True):
        for module in self.model.modules():
            if isinstance(module, LoCPLayer):
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = "none") -> None:
    ...
    if bias == "none":
        ...
        for m in model.modules():
            if isinstance(m, LoCPLayer) and hasattr(m, "bias") and m.bias is not None:
